...
import { STATUS_DELIVERED, STATUS_READ, STATUS_SENT } from "@/models/message";

class ChatService {
  setUpListeners() {
    ConnectyCube.chat.onMessageListener = this.onMessageListener.bind(this);
    ConnectyCube.chat.onSentMessageCallback =
      this.onSentMessageListener.bind(this);
    ConnectyCube.chat.onDeliveredStatusListener =
      this.onDeliveredStatus.bind(this);
  ...
  async sendMessage(dialog, messageText, attachments = false) {
    ...
    const recipient_id =
      dialog.type === DIALOG_TYPE.PRIVATE
        ? dialog.occupants_ids.find((elem) => elem != user.id)
    ...
    if (attachments) {
      return this.sendMessageAsAttachment(
        dialog,
        recipient_id,
        msg,
        attachments
  ...
  async getMessagesAndStore(dialog) {
    ...
    if (
      ...
      const messages = historyFromServer.items.map(
        (elem) => new Message(elem, user.id)
      );
      ...
      if (dialog.unread_messages_count > 0) {
        ...
        this.sendReadStatus(
          firstUnreadMsg.id,
          firstUnreadMsg.sender_id,
          firstUnreadMsg.dialog_id
        );
      }
  ...
  getMoreMessages = async (dialog) => {
    ...
    const messages = moreHistoryFromServer.items.map(
      (elem) => new Message(elem)
    );
    store.dispatch(updateDialog(updateObj));
    const amountMessages = store.dispatch(
      lazyFetchMessages(dialog.id, messages)
    );
  ...
  onMessageListener(senderId, msg) {
    ...
    if (senderId !== user.id) {
      if (dialog === message.dialog_id) {
        ...
        this.sendReadStatus(
          msg.extension.message_id,
          msg.extension.sender_id,
          msg.dialog_id
        );
      } else {
        this.sendDeliveredStatus(
          msg.extension.message_id,
          msg.extension.sender_id,
          msg.dialog_id
        );
  ...
  async createPrivateDialog(userId) {
    ...
    dialogs.forEach((elem) => {
      if (
        elem.type === DIALOG_TYPE.PRIVATE &&
        elem.occupants_ids.find((elem) => elem === userId)
      ) {
        dialog = elem;
      }
    });
  ...
  onSentMessageListener(failedMessage, msg) {
    ...
    store.dispatch(
      updateMessages(msg.extension.dialog_id, msg.id, {
  ...
  onDeliveredStatus(messageId, dialogId, userId) {
    store.dispatch(
      updateMessages(dialogId, messageId, { send_state: STATUS_DELIVERED })
    );
  }
  ...
  async getDialogById(dialogId) {
    ...
    const dialogInRedux = store
      .getState()
      .dialogs.find((elem) => elem.id === dialogId);
    ...
    if (dialogInRedux) {
      ...
      const participantId = dialog.occupants_ids.find(
        (elem) => elem.id !== currentUser
      );
